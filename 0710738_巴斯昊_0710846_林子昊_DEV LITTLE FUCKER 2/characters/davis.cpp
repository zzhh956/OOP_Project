...
Davis::Davis(){
	...
	wa_d_r_a=640;ha_d_r_a=80;pn_d_r_a=8;wn_d_r_a=8;hn_d_r_a=1;bkcolor_d_r_a=15;//往右上鉤拳技能圖檔切割數據
	wa_d_l_a=640;ha_d_l_a=80;pn_d_l_a=8;wn_d_l_a=8;hn_d_l_a=1;bkcolor_d_l_a=15;//往左上鉤拳技能圖檔切割數據

	wa_b_r_a=400;ha_b_r_a=80;pn_b_r_a=5;wn_b_r_a=5;hn_b_r_a=1;bkcolor_b_r_a=15;//往右投擲球技能圖檔切割數據
	wa_b_l_a=400;ha_b_l_a=80;pn_b_l_a=5;wn_b_l_a=5;hn_b_l_a=1;bkcolor_b_l_a=15;//往左投擲球技能圖檔切割數據

	wa_b_r=328;ha_b_r=141;pn_b_r=12;wn_b_r=4;hn_b_r=3;bkcolor_b_r=15;//往右投擲球技能圖檔切割數據
	wa_b_l=328;ha_b_l=141;pn_b_l=12;wn_b_l=4;hn_b_l=3;bkcolor_b_l=15;//往左投擲球技能圖檔切割數據
//--------------------------------------------------------------------------------------存圖檔名稱 
...
//------------------------------------------------------------------------------------------------各種flag 
		...
		if(button_down[5]==1&&button_down[0]==1&&direction[1]==3&&flag_holly_action_right==0
		&&flag_down==0&&flag_defense==0)//判斷上鉤拳出招情況 
			flag_holly_action_left=1;					
		if(button_down[5]==1&&button_down[0]==1&&direction[1]==4&&flag_holly_action_left==0
		&&flag_down==0&&flag_defense==0)//判斷上鉤拳出招情況 
			flag_holly_action_right=1;
		
		if(button_down[5]==1&&button_down[1]==1&&direction[1]==3&&flag_ball_right==0&&flag_holly_action_right==0
		...
		if(button_down[5]==1&&button_down[1]==1&&direction[1]==4&&flag_ball_left==0&&flag_holly_action_left==0
		&&number_of_ball<2&&flag_down==0&&flag_defense==0){//判斷投擲球出招情況 
			if(flag_manyball_remain==1&&flag_ball_record_receive==0)
				skills_record();
			flag_ball_action=1;
...
void Davis::print(){
        if(button_count==0&&direction[1]==4&&flag_holly_action_right==0&&flag_ball_action==0&&flag_down==0&&flag_defense==0)//沒有動作且方向為右時 
        	putimage(position_x,position_y, picture_right[picture_right_count], OR_PUT);
        else if(button_count==0&&direction[1]==3&&flag_holly_action_left==0&&flag_ball_action==0&&flag_down==0&&flag_defense==0)//沒有動作且方向為左時 
...
void Davis::getdata(int &p2_position_x,int &p2_position_y,int &p2_skill1_position_x,int &p2_skill1_position_y,int &p2_skill2_position_x,
...
int &p2_button_down){
	...
	if(flag_holly_action_right==1||flag_holly_action_left==1){
...
void Davis::setdata(int down_switch,int defense_switch){
	if(down_switch==1)
		flag_down=1;
	if(defense_switch==1)
		flag_defense=1;

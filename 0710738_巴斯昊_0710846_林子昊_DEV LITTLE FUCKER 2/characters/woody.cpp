...
Woody::Woody(){
	...
	wa_b_r_a=800;ha_b_r_a=80;pn_b_r_a=10;wn_b_r_a=10;hn_b_r_a=1;bkcolor_b_r_a=15;//往右投擲球技能圖檔切割數據
	wa_b_l_a=800;ha_b_l_a=80;pn_b_l_a=10;wn_b_l_a=10;hn_b_l_a=1;bkcolor_b_l_a=15;//往左投擲球技能圖檔切割數據

	wa_b_r=328;ha_b_r=249;pn_b_r=12;wn_b_r=4;hn_b_r=3;bkcolor_b_r=15;//往右投擲球技能圖檔切割數據
	wa_b_l=328;ha_b_l=249;pn_b_l=12;wn_b_l=4;hn_b_l=3;bkcolor_b_l=15;//往左投擲球技能圖檔切割數據
//--------------------------------------------------------------------------------------存圖檔名稱 
	strcpy(filename_right,".\\roles_picture\\Woody\\woody_0.bmp");
	filename_right[strlen(".\\roles_picture\\Woody\\woody_0.bmp")]='\0';

	strcpy(filename_left,".\\roles_picture\\Woody\\woody_0_mirror.bmp");
	filename_left[strlen(".\\roles_picture\\Woody\\woody_0_mirror.bmp")]='\0';
//		
	strcpy(filename_kick_right_action,".\\roles_picture\\Woody\\kick.bmp");
	filename_kick_right_action[strlen(".\\roles_picture\\Woody\\kick.bmp")]='\0';

	strcpy(filename_kick_left_action,".\\roles_picture\\Woody\\kick_mirror.bmp");
	filename_kick_left_action[strlen(".\\roles_picture\\Woody\\kick_mirror.bmp")]='\0';	
//		
	strcpy(filename_ball_right_action,".\\roles_picture\\Woody\\throw_ball.bmp");
	filename_ball_right_action[strlen(".\\roles_picture\\Woody\\throw_ball.bmp")]='\0';

	strcpy(filename_ball_left_action,".\\roles_picture\\Woody\\throw_ball_mirror.bmp");
	filename_ball_left_action[strlen(".\\roles_picture\\Woody\\throw_ball_mirror.bmp")]='\0';	
//
	strcpy(filename_ball_right,".\\roles_picture\\Woody\\woody_ball.bmp");
	filename_ball_right[strlen(".\\roles_picture\\Woody\\woody_ball.bmp")]='\0';	

	strcpy(filename_ball_left,".\\roles_picture\\Woody\\woody_ball_mirror.bmp");
	filename_ball_left[strlen(".\\roles_picture\\Woody\\woody_ball_mirror.bmp")]='\0';
//-----------------------------------------------------------------------------------------
	...
	ball_left_action_count=9;//往左投擲球圖片初始狀態
...
void Woody::setposition(int x,int y){
	position_x=x;
	position_y=y;
}
void Woody::control(){
	    if(GetAsyncKeyState(up6_key)==0) button_down[0]=0;//上 
        if(GetAsyncKeyState(do6_key)==0) button_down[1]=0;//下 
        if(GetAsyncKeyState(le6_key)==0) button_down[2]=0;//左 
        if(GetAsyncKeyState(ri6_key)==0) button_down[3]=0;//右 
        if(GetAsyncKeyState(at6_key)==0) button_down[4]=0;//攻 
        if(GetAsyncKeyState(sk6_key)==0) button_down[5]=0;//技能 
        
        if(GetAsyncKeyState(up6_key)!=0) button_down[0]=1;//上 
        if(GetAsyncKeyState(do6_key)!=0) button_down[1]=1;//下 
        if(GetAsyncKeyState(le6_key)!=0) button_down[2]=1;//左 
        if(GetAsyncKeyState(ri6_key)!=0) button_down[3]=1;//右 
        if(GetAsyncKeyState(at6_key)!=0) button_down[4]=1;//攻 
        if(GetAsyncKeyState(sk6_key)!=0) button_down[5]=1;//技能 
//------------------------------------------------------------------------------------------------各種flag 
...
void Woody::print_kick(){
        if(flag_kick_action_right==1){//技能被按且方向為右(動作部分)(角色只會做這踢擊動作，按其他按鍵會無反應)
			if(kick_right_action_count<=9){//若動作還沒做完 
			   	position_x += 20;
				putimage(position_x,position_y, kick_right_action_picture[kick_right_action_count], OR_PUT);
		...
		if(flag_kick_action_left==1){//技能被按且方向為左(動作部分)(角色只會做這踢擊動作，按其他按鍵會無反應)
			if(kick_left_action_count>=0){//若動作還沒做完 
				position_x -= 20;
			   	putimage(position_x,position_y, kick_left_action_picture[kick_left_action_count], OR_PUT);
...
void Woody::print_ball(){
	...
	if(flag_ball_action==1&&flag_ball_left==1){//技能被按且方向為左(動作部分)(角色只會做這投擲球動作，按其他按鍵會無反應)
		...
		if(flag_ball_right==1){//球向右飛時的 ball_right_count
			ball_right_count++;
			if(ball_right_count>3)
				ball_right_count=8;
			else if(ball_right_count>11)
				ball_right_count=0;
...
void Woody::print_manyball(){
	...
	if(flag_manyball_remain==1&&flag_ball_past==2&&number_of_ball>=1&&flag_ball_record_receive==1){
		if(manyball_position_x<=last_manyball_position_x){
			...
			if(manyball_right_count>3)
				manyball_right_count=8;
			else if(manyball_right_count>11)
...
void Woody::getdata(int &p2_position_x,int &p2_position_y,int &p2_skill1_position_x,int &p2_skill1_position_y,int &p2_skill2_position_x,
...
void Woody::setdata(int down_switch,int defense_switch){
	if(down_switch==1)
		flag_down=1;
	if(defense_switch==1)
		flag_defense=1;
